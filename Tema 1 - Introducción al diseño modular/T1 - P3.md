...
Vereis que no es necesario agrupar estos atributos con una *struct*, por que ya estan agrupados para formar un unico tipo de datos dentro de la clase. A demás, esta prohibido el uso del *using namespace std*, ya que esto haria publicas todas las funciones (incluso las que hay en privado).
